...
function extractResultId(def: ScrapedRecipeDef, fallbackId: string): { id: string; qty: number } | null {
...
async function upsertCraftingRecipeForItem(itemId: string, def: ScrapedRecipeDef) {
  ...
  if (existingRecipe.length === 0) {
    await db.insert(recipes).values({ id: recipeId, type: "crafting", isBlueprintLocked, inRaid });
  } else {
    await db
      .update(recipes)
  ...
  // Link recipe to item (output) if matches this item
  if (outputId === itemId) {
    await db.update(items).set({ recipeId }).where(eq(items.id, itemId));
  }
  ...
  for (const [wbId, tier] of Object.entries(workbenches)) {
    ...
    await db.insert(workbenchRecipes).values({ workbenchId: wbId, workbenchTier: tierNum, recipeId });
  }

  return { created: existingRecipe.length === 0, updated: existingRecipe.length > 0, skipped: false } as const;
}
