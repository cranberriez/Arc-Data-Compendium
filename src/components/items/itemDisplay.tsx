...
type ItemCardProps = { item?: BaseItem; variant?: "default" | "icon" };
	...
	return (
		...
		>
			{variant === "icon" ? (
				<div
					className={cn(
						"flex items-center justify-center rounded-md h-full w-full border-2 p-2",
						getRarityColor(displayItem.rarity, "border")
					)}
				>
					{displayItem.icon && (
						<displayItem.icon
							className={cn("w-8 h-8", getRarityColor(displayItem.rarity, "text"))}
						/>
					)}
				</div>
			) : (
